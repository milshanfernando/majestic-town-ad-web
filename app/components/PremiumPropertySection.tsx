...
const PremiumPropertySection = ({ property, reverse = false }: Props) => {
  ...
  // useGSAP automatically provides a context, no need for gsap.context
  useGSAP(
    () => {
      // Info card animation
      gsap.from(".info-card", {
      ...
      gsap.utils.toArray<HTMLElement>(".image-grid-item").forEach((el, i) => {
        gsap.from(el, {
          opacity: 0,
          y: 40,
          duration: 0.8,
    ...
    { scope: containerRef, dependencies: [reverse] },
  );

  return (
    <section ref={containerRef} className="w-full py-24 px-6 bg-gray-50">
      <div className="max-w-7xl mx-auto">
        ...
        >
          {/* Left: Pinterest-style Image Grid */}
          <div className="grid grid-cols-2 gap-4 lg:gap-6">
            {property.gallery.map((img, idx) => (
              <div
                key={idx}
                className="relative h-[250px] sm:h-[300px] md:h-[350px] rounded-3xl overflow-hidden image-grid-item cursor-pointer"
              >
                <Image
                  src={img}
                  alt={`Gallery ${idx + 1}`}
          ...
          {/* Right: Info Card */}
          <div className="bg-white p-12 rounded-3xl shadow-2xl info-card flex flex-col justify-between">
            <div>
